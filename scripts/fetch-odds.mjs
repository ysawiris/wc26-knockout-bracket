...
/* ============================================================
   ...
   KNOCKOUT rounds (via ESPN's public scoreboard API) and writes
   the compact data/odds.json the site reads.

   Knockout fork of the group-stage hub's fetcher: same odds math,
   retuned to the knockout date window (Round of 32 → Final) and the
   32-country knockout FIELD for name canon.

   For each tournament day it pulls the scoreboard, takes the first
   odds entry per event (DraftKings, priority 1), and converts the
   O/U line + American prices into a market-implied expected-goals
   total (vig-stripped Poisson inversion).

   Team names are mapped to the exact spellings used in js/data.js
   (the FIELD) so js/matchcenter.js can match a fixture to its line
   by the normalized home/away pair.
...
/* Knockout runs 2026-06-28 (R32) through 2026-07-19 (Final). A day of
   padding each side keeps a late/early kickoff from slipping the net.
   (UTC month is 0-based: June = 5, July = 6.) */
const FIRST_DAY = Date.UTC(2026, 5, 27);
const LAST_DAY = Date.UTC(2026, 6, 20);
...
/* The 32 Round-of-32 countries exactly as they appear in js/data.js
   (the FIELD). matchcenter.js matches a fixture to its line by the
   normalized home/away pair, so these spellings must be byte-for-byte
   the FIELD names. */
const SEED_NAMES = [
  "Argentina", "Spain", "France", "England",
  "Portugal", "Brazil", "Morocco", "Netherlands",
  "Belgium", "Germany", "Croatia", "Colombia",
  "Mexico", "Senegal", "United States", "Japan",
  "Switzerland", "Ecuador", "Austria", "Australia",
  "Algeria", "Egypt", "Canada", "Norway",
  "Ivory Coast", "Sweden", "Paraguay", "DR Congo",
  "South Africa", "Cape Verde", "Ghana", "Bosnia & Herzegovina"
];

/* Known API spellings that don't canon-collapse onto a FIELD name. */
const ALIASES = {
...
/* Raw API name -> FIELD name; unmapped names fall back to their canon
   form so the client's name matching still has a fair shot. */
